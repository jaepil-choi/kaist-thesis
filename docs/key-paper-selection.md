...
### 현재 관심 우선순위 (2026-07-17)

현재 가장 관심이 가는 후보는 아래 세 편이다. #3은 공식 출판사 abstract,
#5와 #9는 OpenAlex abstract를 기준으로 1차 내용을 정리했으며, paper PDF와
Markdown을 확보하면 변수 정의, 표본 구성, 식별전략, 핵심 table과 한국
replication 설계를 다시 정밀 검토한다.

1. **#5 Machine-learning the skill of mutual fund managers** — 펀드 특성으로
   미래 risk-adjusted performance를 예측하는 ML 기반 fund skill 연구.
2. **#9 Mutual Fund Strategy: Swing for the Fences or Bat for Average** —
...
3. **#3 Mutual fund performance: Using bespoke benchmarks to disentangle
   ...
   맞춤형 benchmark로 진정한 skill을 분리하는 연구.

이 우선순위는 `kwam-report-automation/temp/data-exploration`에서 확인한
제로인 펀드 DW의 실제 가용성도 반영한다. 일별 fund return·BM return·AUM,
월별 holdings, 약관상 투자한도·전략, 공식 BM, 스타일, 보수, 운용사 단위
성과를 결합할 수 있어 세 논문 모두 한국 공모펀드 replication 후보로 현실성이
높다.
...
**상태: 현재 관심 우선후보 — PDF/Markdown 정독 대기.**
...
**데이터·재현성**: 중저위험으로 수정. 제로인 DW 탐색 결과, 원 논문의 핵심
입력에 직접 대응하는 자료가 사내에 존재한다. `DW_ZI_펀드약관한도`에는
국내주식·해외주식·채권·유동성·파생상품·레버리지 등의 최소/최대 투자비율이
정형화되어 있고, `DW_ZI_펀드약관투자`에는 투자목적·전략·위험 관련 자유서술이
있다. `DW_ZI_펀드기본정보`의 공식 BM과 편입한도,
`DW_ZI_펀드스타일분석`의 size/value-growth·PER/PBR·beta,
`DW_ZI_펀드자산내역`의 월별 holdings를 결합하면 bespoke benchmark 구성에
필요한 mandate·constraint·실제 portfolio를 상당 부분 재현할 수 있다.
다만 약관·기본정보 table에 기준일자가 없어 과거 mandate/BM 변경 이력이
보존되는지는 별도 확인이 필요하고, holdings table의 역사적 coverage와
survivorship-bias-free universe 구성도 PDF 정독 후 검증해야 한다.

**원문 검토 상태**: 현재 내용은 ScienceDirect의 공식 abstract와 데이터
schema에 기초한 1차 정리다. PDF/Markdown 확보 후 bespoke factor 구성법,
constraint coding, 성과 순위 변화의 경제적 크기와 robustness test를 보강한다.
...
**데이터·재현성**: 저위험이며, 제로인 DW 확인 후 더 강하게 평가한다.
...
변수에 대응한다. `DW_ZI_펀드약관보수`, `DW_ZI_클래스펀드`, 설정일·해지일을
이용하면 fee, share-class 통합, fund age와 생존편향도 처리할 수 있다. 원
논문의 abstract상 holdings-stock characteristics는 미래 성과를 예측하지
못하므로, holdings가 없어도 핵심 replication이 가능하다는 점도 장점이다.
사전 계산된 alpha·IR은 검산용으로 쓰고, 최종 논문에서는 raw fund/BM return으로
주요 성과지표를 직접 재계산하는 편이 재현성 측면에서 안전하다.

**원문 검토 상태**: 현재 내용은 OpenAlex abstract에 기초한 1차 정리다.
PDF/Markdown 확보 후 predictor set, neural-network 구조, 학습/검증 window,
long-short portfolio construction, 거래비용·수수료 처리와 sentiment interaction을
한국 표본에 맞게 구체화한다.
...
**데이터·재현성**: 저위험. KOSPI200 편입·편출 이벤트, 가격·거래량 모두
KRX 공개 데이터만으로 충분 — 후보군 중 가장 clean한 데이터 요구사항.
...
**데이터·재현성**: 저위험. 지수 구성종목 이력·가중치는 유료 벤더(FnGuide/
DataGuide 등)로 표준적으로 구할 수 있음.
...
**한국 replication 아이디어**: 월별 펀드 holdings와 국내주식 가격·스타일
...
**데이터·재현성**: 중저위험. `DW_ZI_펀드자산내역`의 월별 종목 holdings,
`DW_ZI_펀드일별분석`의 설정액·순자산·수익률·BM수익률,
`DW_ZI_펀드스타일분석`의 size/value-growth와 beta,
`DW_ZI_펀드약관보수`의 fee를 결합하면 abstract에 나타난 핵심 분석 대부분에
대응할 수 있다. 국내 주식 가격·재무자료도 별도로 확보 가능하다. 다만 원 논문의
shareholder-report 종목 언급 메커니즘을 재현하려면 국내 자산운용보고서
텍스트의 역사적 archive와 종목명 추출 가능성을 추가 확인해야 한다. 이
메커니즘을 제외하더라도 SF/BA 측정과 flow·risk·performance 분석이라는 핵심
replication은 가능해 보인다.

**원문 검토 상태**: 현재 내용은 OpenAlex abstract에 기초한 1차 정리다.
PDF/Markdown 확보 후 SF·BA의 정확한 수식, style-adjustment 기준, holdings
lag, active/passive 표본 정의, flow·fee·risk 회귀식과 falsification test를
한국 데이터 설계에 맞춰 보강한다.
