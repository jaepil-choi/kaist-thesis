...
def test_frictions_policy_accepts_previous_weight() -> None:
    model = CNNTransformerFrictions(random_seed=7, lookback=30)
    inputs = torch.linspace(-0.1, 0.1, 90).reshape(3, 30)
    outputs = model(inputs, torch.tensor([0.2, -0.1, 0.0]))
