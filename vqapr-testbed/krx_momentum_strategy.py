...
STRATEGY_ID = "krx_momentum_strategy"
...
class KrxMomentumStrategy(StrategyModel):
    ...
    def on_occurrence(self, context):
        ...
        for row in rows:
            ...
            if value is not None:
                # Source parquet stores `close` as float64, not Decimal; the scaffold assumes
                # Decimal rows. Cast via str() to avoid binary-float noise entering the Decimal.
