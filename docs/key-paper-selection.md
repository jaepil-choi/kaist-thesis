...
- **제외 — #3 Mutual fund performance: Using bespoke benchmarks to disentangle
   mandates, constraints and skill (2026-07-18 확정 no-go)** — 펀드별 mandate와
   constraint를 반영한 맞춤형 benchmark로 진정한 skill을 분리하는 연구.
   30개 domestic active equity fund pilot whitelist로 `DW_ZI_펀드약관한도`·
   `DW_ZI_펀드약관투자`·`DW_ZI_펀드스타일분석`을 실제로 추출해 constraint-coding
   gate를 직접 테스트했다. 자산군별 투자대상 한도(9개 `투자대상구분코드`)와
   스타일 9-box는 30개 전부에서 안정적으로 채워지지만, 차입/레버리지·공매도·
   회전율은 30개 전부에서 구조화 필드에도 자유서술 원문에도 나타나지 않았고
   증권대여는 1/30, 파생상품은 6/30에서 수치 없는 서술로만 등장했다. `docs/
   candidate-paper-data-inventory.md` §7의 "차입·공매도·대여·회전율 중 최소
   세 가지를 수치화할 수 있어야 함"이라는 자체 진행조건을 충족하지 못해
   같은 문서 §8의 규칙에 따라 이 후보를 제외한다.
...
**2026-07-18 갱신**: #3 Beber의 constraint-coding gate를 30-fund pilot
실측으로 직접 테스트한 결과 실패로 확정됐다(위 §3 상세 및 요약 목록 참고).
따라서 아래 두 문단의 "#3은 자유서술 제약 코딩과 표본 coverage가 통과해야
하는 조건부 후보다"는 서술은 더 이상 유효하지 않다 — #3은 조건부가 아니라
제외로 확정한다.
...
**상태: 30-fund pilot 실측 완료 — constraint-coding gate 실패, 제외 확정 (2026-07-18).**

이전에는 원문 정독·Priority 4~5 metadata 확인만으로 "조건부 3순위"로
분류했으나, 이후 30개 domestic active equity fund whitelist로 `DW_ZI_
펀드약관한도`·`DW_ZI_펀드약관투자`·`DW_ZI_펀드스타일분석`을 실제로 추출해
gate를 직접 테스트했다. 결과는 no-go다: 자산군 투자대상 한도와 스타일
9-box는 30개 전부에서 잘 채워지지만, 차입/레버리지·공매도·회전율은
0/30, 증권대여는 1/30(수치 없는 서술), 파생상품은 6/30(수치 없는 서술)만
확인됐다. 아래 원래의 "데이터·재현성" 평가(Priority 4~5 metadata 기반
추정)는 이 실측으로 대체된다 — 이 절 마지막의 실측 상세 참고.
...
**30-fund pilot 실측 결과 (2026-07-18)**: share-class 중복 제거, 10년+
설정 이력, 매니저·스타일 분산 조건으로 선정한 30개 whitelist(일반/가치/
성장/중소형/배당/테마 6개 스타일 버킷, 19개 서로 다른 운용사)로
`docs/company-data-extraction-request.md` §6의 세 소스를 추출했다.

- `약관한도` 270행: 30개 펀드 전부 동일하게 9개 `투자대상구분코드`만
  존재(`FB10/11/12` 국내주식, `FB20/21/22` 채권, `FB31/32/33` 유동성자산).
  이전에 존재 가능성만 언급했던 `FB41~43`(파생상품 한도), `FB53`
  (차입·레버리지 한도) 코드는 30개 중 하나에서도 나타나지 않았다.
- `약관투자` 96행(자유서술 원문) 키워드 스캔: 차입금·레버리지·공매도·
  회전율·스왑 0/30, 대여 1/30(수치 없는 서술), 파생상품 6/30(수치 없는
  서술).
- `스타일분석` 1,030행: 9-box 비중합은 전부 100%(±0.0002%p)로 안정적.

즉 원 논문의 5개 제약(투자대상·공매도·차입/현금·회전율·거래비용) 중
투자대상만 한국 ZI 데이터로 확실히 코딩 가능하고, 나머지는 구조화 필드에도
자유서술에도 없다. `docs/candidate-paper-data-inventory.md` §7의 진행조건
(최소 세 가지 제약 수치화)을 충족하지 못하므로 이 후보는 제외한다.
