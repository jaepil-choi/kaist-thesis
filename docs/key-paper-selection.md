...
**데이터·재현성**: 중~고위험. Bespoke benchmark 구성에는 펀드별 *전체*
보유내역 시계열이 필요한데, 한국 공모펀드 공시(자산운용보고서 등)가 미국
13F만큼 촘촘하고 완전한지 확인이 안 됨 — 진행 전 홀딩스 데이터 완전성부터
검증 필요. Mandate/style 분류는 KOFIA 펀드 분류체계로 대용 가능해 보임.
...
**데이터·재현성**: 8개 후보 중 가장 고위험. 이 논문은 미국 13F(모든
...
**데이터·재현성**: 저위험. 이 논문은 holdings 없이 펀드 수익률·수수료·flow만
쓴다(원 논문에서도 보유종목 characteristic은 예측력이 없었다고 밝힘) — 한국
공모펀드 수익률/NAV/flow는 KOFIA·상업 벤더로 잘 정리되어 있어 #3, #4보다
접근성이 훨씬 좋다.
...
KRX 공개 데이터만으로 충분 — 8개 후보 중 가장 clean한 데이터 요구사항.
...
#8은 펀드 성과평가, 기관투자자, ETF, KOSPI200 편입·편출 등 상위 주제가
