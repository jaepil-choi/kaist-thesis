...
| 제외(2026-07-18) | Beber et al. (2021), *Bespoke Benchmarks* | 중상 | 제외 | 30-fund pilot 실측 결과 자산군 한도·스타일만 코딩 가능. 차입·공매도·대여·turnover는 30개 전부에서 구조화 필드에도 자유서술에도 사실상 없어 constraint-coding gate 실패 |
...
세 번째 후보였던 **Beber는 2026-07-18 실측으로 제외 확정**됐다. 원 논문도 2009년 한 시점의 prospectus 제약을 쓰므로 현재 snapshot만 있다는 사실 자체는 단면 replication과 양립한다. 그러나 30개 domestic active equity fund whitelist로 `DW_ZI_펀드약관한도`·`DW_ZI_펀드약관투자`를 실제로 추출한 결과, ZI에서 숫자로 확인된 것은 30개 전부 동일한 9개 자산군별 최소·최대 한도(`FB10~FB33`)뿐이었다. 차입·공매도·증권대여·회전율은 `FF02` 자유서술 96행 전체를 키워드로 스캔해도 0~1/30 수준(그마저 수치 없는 서술)이라 일관되게 코딩할 수 있는 표본이 없다. 이 절 아래 §4.3의 실측 상세와 §7의 진행조건 미충족 판정을 참고. holdings top-10 한계는 이제 이 판정의 부차적 사유일 뿐이다.
...
**2026-07-18 실측 갱신**: 위 표의 "미확인" 항목들을 30-fund pilot 실제
추출로 확인했다. `약관한도` 270행(30개 펀드 × 9개 `투자대상구분코드`)에는
`FB41~43`(파생상품), `FB53`(차입/레버리지) 행이 하나도 없었다 — "코드만
존재"가 아니라 **이 30개 펀드에서는 아예 나타나지 않음**으로 확정한다.
`약관투자` 96행 자유서술 전체를 키워드 스캔한 결과 차입금/레버리지/공매도/
회전율은 0/30, 증권대여는 1/30(수치 없는 서술), 파생상품은 6/30(수치 없는
서술)이었다. 즉 investment universe(1행)와 cash(1행)를 제외한 나머지
5개 constraint 행은 모두 "구조화 필드 없음"에 더해 "자유서술에서도 실질적
코딩 불가"로 하향 확정한다.
...
**2026-07-18 gate 판정: 실패, 제외.** 30-fund pilot 실측(위 실측 갱신 참고)
결과 차입·공매도·대여·회전율 4개 중 수치화 가능한 항목이 0개였다(대여·
파생상품은 수치 없는 서술로만 1/30, 6/30 등장). §7의 진행조건("최소 세
가지 제약 수치화")을 충족하지 못했으므로 Beber는 이 시점에서 제외하고,
더 이상 장기 표본 확장이나 추가 회사 추출 요청을 진행하지 않는다.
...
**2026-07-18 확정**: 자산군·cash 한도는 30개 전부에서 확인됐지만
short·lending·borrowing·turnover는 `FF02` 자유서술 96행 전체를 스캔해도
0~1/30(수치 없는 서술)에 그쳐 마지막 조건을 충족하지 못했다. 조건부
후보가 아니라 **제외**로 확정한다.
...
현재 데이터 실행확실성 기준 최종 권고는 **Kaniel > Arnott > Ahn(축소)**이며, **Beber는 제외**한다(2026-07-18).
학술지 prestige만으로 tier 4인 Arnott보다 tier 3인 Beber를 앞세우는 방안은
constraint-coding gate 실패로 더 이상 유효하지 않다.
