...
| lookback | 30일 | 30일 (동일) |
| 학습·재학습 | 1,000일 학습, 125일 재학습 | 동일, seed 0, 100 epochs |
...
- 현재 저희 PCA $K>0$ 결과는 **공개코드는 따랐지만 논문 서술과는 어긋난** 상태입니다.
- 고치면 PCA 계열 결과 전체를 재실행해야 합니다. 앞의 4.15도 다시 나옵니다.

::: {.ask}
[여쭙고 싶은 것]{.hd}
공개코드를 그대로 따라가고 논문에 한계로 명시할지, 아니면 논문 본문대로 고치고 재실행할지.
...
1. **PCA look-ahead** — 공개코드를 따를지, 논문 본문대로 고치고 재실행할지
