...
Replication of Hoberg et al. (2018) Table 2 for Korean market

This script implements the return comovement analysis examining whether
focal firm returns are predicted by lagged returns of TNIC and SIC industry peers.

Reference: Hoberg, G., & Phillips, G. (2018). Text-based industry momentum.
...
print("=" * 80)
print("TABLE 2: RETURN COMOVEMENT FAMA-MACBETH REGRESSION")
print("Hoberg et al. (2018) Replication for Korean Market")
print("=" * 80)
...
# - TNIC peers: Equal-weighted (less visible peers)
# - SIC peers: Value-weighted (more visible peers, following Moskowitz & Grinblatt 1999)
tnic_ret = pd.read_parquet(CHECKPOINT_DIR / 'checkpoint_03_tnic_peer_returns.parquet')  # Equal-weighted
...
print(f"  TNIC peer returns (EW): {tnic_ret.shape} (dates x securities)")
...
# TNIC peer returns at different lags (t, t-1, t-2, ..., t-6)
tnic_lags = {}
for lag in range(7):
    var_name = f'tnic_t{lag}' if lag == 0 else f'tnic_t_{lag}'
...
for var_name, var_df in tnic_lags.items():
    ...
    tnic_lags_clean[var_name] = clean_df
    tnic_drops[var_name] = dropped
...
print(f"    TNIC lags:   {sum(tnic_drops.values()):4d} total columns dropped across all lags")
...
# Standardize TNIC peer returns
for var_name, var_df in tnic_lags.items():
    standardized_vars[var_name] = standardize_cross_sectional(var_df, by_date=True)

print(f"  TNIC variables standardized: {list(tnic_lags.keys())}")
...
specifications = {
    'Spec1': {
        ...
        'vars': controls + ['tnic_t0', 'sic_t0']
    },
    'Spec2': {
        'desc': 'Controls + TNIC_t',
        'vars': controls + ['tnic_t0']
    },
    'Spec3': {
        'desc': 'Controls + SIC_t',
    ...
    'Spec4': {
        ...
        'vars': controls + ['tnic_t0', 'tnic_t_1', 'tnic_t_2', 'tnic_t_3',
                           'sic_t0', 'sic_t_1', 'sic_t_2', 'sic_t_3']
    },
    'Spec5': {
        'desc': 'Controls + TNIC_(t to t-6) + SIC_(t to t-6)',
        'vars': controls + ['tnic_t0', 'tnic_t_1', 'tnic_t_2', 'tnic_t_3', 'tnic_t_4', 'tnic_t_5', 'tnic_t_6',
                           'sic_t0', 'sic_t_1', 'sic_t_2', 'sic_t_3', 'sic_t_4', 'sic_t_5', 'sic_t_6']
    },
    'Spec6': {
        'desc': 'Controls + TNIC_(t-1 to t-3) + SIC_(t-1 to t-3)',
        'vars': controls + ['tnic_t_1', 'tnic_t_2', 'tnic_t_3',
                           'sic_t_1', 'sic_t_2', 'sic_t_3']
    },
    'Spec7': {
        'desc': 'Controls + TNIC_(t-1 to t-6) + SIC_(t-1 to t-6)',
        'vars': controls + ['tnic_t_1', 'tnic_t_2', 'tnic_t_3', 'tnic_t_4', 'tnic_t_5', 'tnic_t_6',
...
# TNIC Peer Returns
print("\nTNIC Peer Returns:")
for var in sorted([k for k in coverage_stats.keys() if k.startswith('tnic_')]):
    stats = coverage_stats[var]
    print(f"  {var:<18} {stats['mean']:>11.1f}% {stats['min']:>9.1f}% {stats['max']:>9.1f}%")
...
# This is necessary due to data limitations (TNIC coverage 19%, log_be_me 171 dates)
...
all_vars = ['log_be_me', 'log_size', 'ret_t1', 'ret_t2_t12'] + \
           [f'tnic_t{i}' if i == 0 else f'tnic_t_{i}' for i in range(7)] + \
...
def save_regression_results(results_df, results_dict, specifications_dict,
    ...
    output_prefix : str
        Prefix for output filenames (e.g., 'table2_return_comovement')
    table_title : str
        Title for the table (e.g., 'TABLE 2: RETURN COMOVEMENT')
...
print("\n" + "=" * 80)
print("TABLE 2 RESULTS: RETURN COMOVEMENT")
print("=" * 80)
...
saved_files = save_regression_results(
    ...
    output_prefix="table2_return_comovement",
    table_title="TABLE 2: RETURN COMOVEMENT",
    dependent_var="Month t Own Return",
    se_method="Newey-West (2 lags)"
)
...
# Display results
print("\n" + "=" * 80)
print("TABLE 2 RESULTS: RETURN COMOVEMENT (t+1 DEPENDENT VARIABLE)")
print("=" * 80)
...
saved_files_t1 = save_regression_results(
    ...
    specifications_dict=specifications,
    output_prefix="table2_return_comovement_t1",
    table_title="TABLE 2: RETURN COMOVEMENT (t+1 Prediction)",
...
print("\nKey Findings:")
print("1. Check TNIC peer return coefficients (should be positive and significant)")
print("2. Check SIC peer return coefficients (should weaken after controlling for TNIC)")
print("3. Compare contemporaneous (t) vs lagged (t-1 to t-6) coefficients")
print("4. Examine whether TNIC effects persist longer than SIC effects")
